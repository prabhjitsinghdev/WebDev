...
define (['N/record', 'N/render'], function(record, render){
	function beforeLoad(context){
      ...
      // +'JSON :: ' +context});
		      ...
		      */
			/*
                        var rec = record.load({type:record.Type.SALES_ORDER,id: 4606, isDynamic: true});
		       	rec.addButton({
	...
	function beforeSubmit(context){
		 log.debug({title: 'DEBUG', details: 'BEFORE SUBMIT ::: scriptcontext:' +'JSON :: ' +context});
		// +JSON.stringify(context)});
		//doPrintWork(context); 
	}
	function afterSubmit(context){
		 log.debug({title: 'DEBUG', details: 'AFTER SUBMIT *** scriptcontext:' +'JSON :: ' +context});
		// +JSON.stringify(context)});

	}	
	function doPrintWork(context){
		if(context.type == context.UserEventType.PRINT){
		      log.debug({title: 'DEBUG', details: 'do Print:: scriptcontext:' +'JSON :: ' +context});
			// +JSON.stringify(context)});
		      try{
				var recID = record.id;
